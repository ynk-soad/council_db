...
def create_teireikai_table():
    ...
    for filename in sorted(os.listdir(folder_path)):
        ...
        # --- force_to_december判定 ---
        force_to_december = False
        for line in content_lines[:10]:
            line_n = normalize_text(line)
            if re.search(r"定例会本会議[（(]2[）)]", line_n):
                force_to_december = True
                break
        ...
        temp_meeting_data[key].append({
            ...
            "force_december": force_to_december
        })
    ...
    for (year, session_number, type_teirei), meetings in temp_meeting_data.items():
        ...
        # force_to_decemberが1つでもあれば12月に強制
        if any(m["force_december"] for m in meetings):
            corrected_month = "12"
        else:
            corrected_month = start_date[4:6]
        ID_teirei = f"{type_teirei}{jis_code}{year}{corrected_month}"
